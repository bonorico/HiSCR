...
change <- function(.data, baseline, week16, method = c("fraction", "absolute"))
{
  ...
  switch(method,
         fraction = {
           if (any(.data$baseline) <= 0)
             stop("All baseline values must be positive")
           
           with(.data,
                (week16 - baseline)/baseline
           )
...
#' @param fist_incr - scalar numeric - Negative value for absolute change in number of draining fistula relative to baseline
#' @param abscesses_incr - scalar numeric - Negative value for absolute change in number of abscesses relative to baseline
...
HiSCR <- function(.data, AN_incr = -0.5, fist_incr = 0, abscesses_incr = 0, checkupvs = "HiSCR")
{
  ...
  # returns dataset with new HiSCR
}
